...
class Experiment:
    name: str
    phase: int                  # 0=Baseline, 1=Hyperparam, 2=Dropout, 3=WeightTying
    learning_rate: float
    d_model: int       = 20
    n_heads: int       = 1
    num_layers: int    = 1
...
PHASE_NAMES = {0: "Baseline LR", 1: "Hyperparam opt", 2: "Dropout", 3: "Weight tying"}
...
class ExperimentTracker:
    ...
    def log(
        ...
        exp = Experiment(
            name=name, phase=phase, learning_rate=learning_rate,
            d_model=d_model if d_model is not None else 512,
            n_heads=n_heads if n_heads is not None else 8,
    ...
    def summary(self):
        if not self.experiments:
            ...
            return
        col = "{:<3} {:<28} {:<5} {:<10} {:<8} {:<7} {:<7} {:<7} {:<8} {:<8} {:<10} {:<10} {:<6}"
        header = col.format("#", "Name", "Phase", "LR", "d_model", "heads", "layers",
                            "ff_dim", "dropout", "wt_tie", "Test PPL", "Dev PPL", "Status")
        sep = "─" * len(header)
        ...
        for i, exp in enumerate(self.experiments):
            ...
            print(col.format(
                i+1, exp.name[:27], exp.phase, exp.learning_rate,
                exp.d_model, exp.n_heads, exp.num_layers, exp.ff_dim,
                exp.dropout, str(exp.weight_tying), ppl_str, dev_ppl_str, exp.status,
            ) + (" ◄" if is_best else ""))
        print(sep)
