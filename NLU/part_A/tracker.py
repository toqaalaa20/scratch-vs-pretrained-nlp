...
class Experiment:
    name: str
    phase: int                  # 0=Baseline LR, 1=Hyperparam opt, 2=Dropout
    learning_rate: float
    d_model: int       = 20
    n_heads: int        = 1
    num_layers: int     = 1
...
PHASE_NAMES = {0: "Baseline LR", 1: "Hyperparam opt", 2: "Dropout"}


class ExperimentTracker:
    def __init__(self, save_dir: str = "NLU/part_A/results"):
    ...
    def log(
        ...
        exp = Experiment(
            ...
            d_model=d_model if d_model is not None else 20,
            n_heads=n_heads if n_heads is not None else 1,
    ...
    def summary(self):
        ...
        for i, exp in enumerate(self.experiments):
            ...
            print(col.format(
                i+1, exp.name[:31], exp.phase, exp.learning_rate,
                exp.d_model, exp.n_heads, exp.num_layers, exp.ff_dim,
                exp.dropout, f1_str, acc_str, exp.status,
            ) + (" <-" if is_best else ""))
        print(sep)
